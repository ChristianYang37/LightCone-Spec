...
@dataclass(frozen=True)
class ConfirmationBlockPlan:
    """Power-locked activation of a prefix of the 20 final block candidates."""
    ...
    def __post_init__(self) -> None:
        if self.experiment not in {"E3b", "E5"}:
            raise ValueError("confirmation block plans are defined only for E3b/E5")
        for name in (
            "registry_sha256",
            "runtime_sha256",
            "split_sha256",
        ...
        if self.status == "POWERED":
            if (
                ...
                or not 12 <= self.selected_final_blocks <= MAXIMUM_FINAL_BLOCKS
            ):
                raise ValueError("POWERED plans require 12--20 final blocks")
        elif self.status == "UNDERPOWERED":
            if self.selected_final_blocks is not None:
                raise ValueError("UNDERPOWERED plans cannot activate final blocks")
        else:
            raise ValueError("block-plan status must be POWERED or UNDERPOWERED")
        if not isinstance(self.reason_code, str) or not _REASON_CODE(self.reason_code):
            raise ValueError("block-plan reason_code is invalid")
    ...
    @property
    def activated_blocks(self) -> tuple[int, ...]:
        if self.status == "UNDERPOWERED":
            return PILOT_BLOCKS
        assert self.selected_final_blocks is not None
        return PILOT_BLOCKS + FINAL_BLOCKS[: self.selected_final_blocks]
...
class DispatchWave:
    ...
    def __post_init__(self) -> None:
        if len(self.cells) not in {1, 2}:
            raise ValueError("a two-GPU dispatch wave contains one or two cells")
        if len(self.cells) == 1:
            return
        first, second = self.cells
        if first.resources.exclusive or second.resources.exclusive:
            raise ValueError("exclusive cells cannot share a dispatch wave")
        if set(first.resources.gpu_uuids) & set(second.resources.gpu_uuids):
            raise ValueError("concurrent cells cannot share a GPU")
        if set(first.resources.ports) & set(second.resources.ports):
            raise ValueError("concurrent cells cannot share ports")
        if first.resources.cache_root == second.resources.cache_root:
            raise ValueError("concurrent cells cannot share cache roots")
        if first.resources.evidence_root == second.resources.evidence_root:
            raise ValueError("concurrent cells cannot share evidence roots")

    @property
    def sha256(self) -> str:
        return content_sha256(tuple(cell.cell_id for cell in self.cells))


@dataclass(frozen=True)
class TwoGpuResourceScheduler:
    """Pure deterministic planner for the registry's already-bound resources."""
    ...
    def schedule(
        ...
        *,
        ...
        interference_gate_passed: bool = False,
        confirmation_block_plan: ConfirmationBlockPlan | None = None,
        activation_plan: StageActivationPlan | None = None,
    ) -> tuple[DispatchWave, ...]:
        completed = tuple(completed_cell_ids)
        if len(completed) != len(set(completed)):
            raise ValueError("completed cell identities must be unique")
        known_cell_ids = {cell.cell_id for cell in self.registry.cells}
        if set(completed) - known_cell_ids:
        ...
        if experiment is None:
            return ()
        activated_cell_ids: set[str] | None = None
        if experiment in {"E1", "E2"}:
            activated_cell_ids = self._validate_activation_plan(
                experiment,
                receipts,
                activation_plan,
            )
        ...
        activated_blocks: tuple[int, ...] | None = None
        if experiment in {"E3b", "E5"}:
            activated_blocks = self._activated_confirmation_blocks(
                experiment,
                completed,
                confirmation_block_plan,
            )
        elif confirmation_block_plan is not None:
            raise ValueError("confirmation block plan does not match the ready stage")
        pending = [
            cell
            for cell in self.registry.cells_for(experiment)
            if self._dispatchable(cell)
            and cell.cell_id not in completed
            and (activated_cell_ids is None or cell.cell_id in activated_cell_ids)
            and (activated_blocks is None or cell.identity.block in activated_blocks)
        ]
        pending.sort(key=self._dispatch_order_key)
        waves: list[DispatchWave] = []
        while pending:
            first = pending.pop(0)
            if first.resources.exclusive or not interference_gate_passed:
                waves.append(DispatchWave((first,)))
                continue
            partner_index = next(
                (
                    index
                    for index, candidate in enumerate(pending)
                    if self._compatible(first, candidate)
                ),
                None,
            )
            if partner_index is None:
                waves.append(DispatchWave((first,)))
                continue
            second = pending.pop(partner_index)
            waves.append(DispatchWave((first, second)))
        return tuple(waves)
    ...
    def _validate_activation_plan(
        ...
        if disposition_ids != set(stage_cells):
        ...
        if any(
            serving_cell_rejection_reason(stage_cells[cell_id]) is not None
            for cell_id in plan.activated_cell_ids
        ):
            raise ValueError("activation plan cannot enable an unresolved serving cell")
        base_blocked = {
        ...
        if experiment == "E2":
            match = re.fullmatch(r"halving_([0-3])", plan.activation_round)
            if match is None:
                raise ValueError(
                    "E2 activation round must name halving_0 through halving_3"
                )
            stage_index = int(match.group(1))
            marker = f"halving_stage={stage_index}:"
            if any(
                marker not in stage_cells[cell_id].identity.variant
                for cell_id in plan.activated_cell_ids
            ):
            ...
            adaptive_pairs: dict[str, set[str]] = {}
            adaptive_gpus: dict[str, set[tuple[str, ...]]] = {}
            for cell in activated:
                if cell.identity.method not in {"tts", "l0"}:
                    continue
                pair_id = _adaptive_pair_id(cell.identity)
                adaptive_pairs.setdefault(pair_id, set()).add(cell.identity.method)
                adaptive_gpus.setdefault(pair_id, set()).add(cell.resources.gpu_uuids)
            if not adaptive_pairs or any(
                methods != {"tts", "l0"} for methods in adaptive_pairs.values()
            ):
                raise ValueError("E2 activation must retain matched TTS/L0 pairs")
            if any(len(gpus) != 1 for gpus in adaptive_gpus.values()):
                raise ValueError("E2 matched TTS/L0 pairs must use the same GPU")
        elif experiment == "E1":
            ...
            if len(widths) != 1 or len(concurrencies) != 1:
                raise ValueError(
                    "E1 activation must select exactly one width/load slice"
                )
            if {cell.identity.method for cell in activated} != {
                ...
                raise ValueError(
                    "E1 activation must retain both baselines and both publication policies"
                )
        return set(plan.activated_cell_ids)

    def _activated_confirmation_blocks(
        self,
        experiment: str,
        completed: tuple[str, ...],
        plan: ConfirmationBlockPlan | None,
    ) -> tuple[int, ...]:
        if plan is None:
            return PILOT_BLOCKS
        if (
            ...
            raise ValueError("confirmation block plan identity mismatch")
        pilot_ids = tuple(
            sorted(
                cell.cell_id
                for cell in self.registry.cells_for(experiment)
                if self._dispatchable(cell) and cell.identity.block in PILOT_BLOCKS
            )
        )
        if not pilot_ids:
            raise ValueError("confirmation stage has no dispatchable pilot cells")
        if not set(pilot_ids) <= set(completed):
            raise ValueError("final blocks cannot activate before every pilot cell")
        if plan.completed_pilot_cells_sha256 != content_sha256(pilot_ids):
            raise ValueError("confirmation plan does not bind the exact pilot cells")
        return plan.activated_blocks

    @staticmethod
    def _compatible(first: ExperimentCell, second: ExperimentCell) -> bool:
        if first.resources.exclusive or second.resources.exclusive:
            return False
        if (
            first.resources.workload_class is WorkloadClass.HEADLINE
            or second.resources.workload_class is WorkloadClass.HEADLINE
        ) and (
            first.resources.workload_class is not second.resources.workload_class
            or first.identity.block == second.identity.block
        ):
            return False
        return (
            not set(first.resources.gpu_uuids) & set(second.resources.gpu_uuids)
            and not set(first.resources.ports) & set(second.resources.ports)
            and first.resources.cache_root != second.resources.cache_root
            and first.resources.evidence_root != second.resources.evidence_root
        )
    ...
    def _dispatch_order_key(self, cell: ExperimentCell) -> tuple[int, int, str, str]:
        """Randomize methods inside a block while keeping pilots first."""
        identity = cell.identity
        phase = 0 if identity.block in PILOT_BLOCKS else 1
        paired_group = content_sha256(
            {
                "registry_seed": self.registry.cells[0].identity.seed,
        ...
        return phase, identity.block, paired_group, method_order
...
def _adaptive_pair_id(identity: CellIdentity) -> str:
    """Identify a TTS/L0 pair while excluding publication policy and GPU."""
...
def build_industrial_registry(
    *,
    gpu_uuids: tuple[str, str] = ("GPU-0", "GPU-1"),
    base_port: int = 24000,
