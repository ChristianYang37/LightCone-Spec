...
def update_qa_job(source, case):
    ...
    job = replace(source, job_id=f"excluded-preview-v3-{case}", node="excluded-preview-v3-updates",
        method=method, load="c1", gpu_count=1, block=None,
        parameters={**source.parameters, "preview_revision": 3, "frozen_recipe": recipe,
            "stride": recipe["stride"], "topology": "tp1_dp1", "excluded_from_analysis": True,
...
def main():
    ...
    if args.optimizer_only:
        ...
        config = OnlineAdaptationConfig.load(args.optimizer_only, tp_size=1, dp_size=1)
    ...
    job, recipe = update_qa_job(source, args.case)
    payload = adaptation_payload(job, recipe)
    (args.output / "adaptation.json").write_text(json.dumps(payload, indent=2))
    (args.output / "config.json").write_text(json.dumps({"job": job.to_dict(), "gpu": args.gpu,
    ...
    try:
        ...
        with process as client:
            ...
            for name, tokens, temperature in plan:
                ...
                if args.case == "ensemble":
                    online = before["online_spec_state"]
                ...
                if args.case == "ensemble":
                    assert after["online_spec_state"]["step"] > 0
